...
function _frame(Scrubber,d3,frameCount,delay){return(
  Scrubber(d3.range(1, frameCount, 1), {
    loop: false,
    autoplay: false,
    delay
  })
  ...
  function _chart(d3,width,height,xAxis,x,yAxis,y,Tooltip,margin,getDataSlice,getPDataSlice,delay,defaultXDomain,genLine,bisect)
  {
    ...
    return Object.assign(svg.node(), {
      update(frame) {
        ...
        let xRangeShift =
          horizScale(newDomainX[1]) - horizScale(newDomainX[1] - delay);
        ...
        gy.transition(t).call(
          yAxis,
          y.domain([0, d3.max(newdata, (d) => d.latency) + 100]).nice()
        );
        ...
        transformG
          ...
          .attr("transform", null)
          .transition(t)
          .attr("transform", `translate(-${xRangeShift}, 0)`);
        ...
        let tooltipCb = (mouseAt) =>
          tooltip.show(
            bisect(newdata, horizScale.invert(mouseAt)),
            horizScale,
            t
          );
        ...
        function zoomed(event) {
          ...
          line = genLine(xz);
          transformG
            .selectAll("circle:not(#tooltip-cir)")
            .attr("cx", (d) => xz(d.ts));
  ...
  function _3(chart,frame){return(
  chart.update(frame)
  )}
  ...
  function _pDataSamplingRate(){return(
  4
  )}
  
  function _chartFrameLimit(){return(
  200
  )}
  
  function _frameCount(){return(
  10000
  )}
  ...
  function _renderForwardBy(){return(
  4
  )}
  ...
  function _defaultXDomain(now,delay,chartFrameLimit){return(
  [now - delay * chartFrameLimit, now]
  )}
  
  function _defaultYDomain(){return(
  [0, 100]
  )}
  
  function _x(d3,defaultXDomain,margin,width){return(
  d3
    .scaleTime()
  ...
  function _y(d3,defaultYDomain,height,margin){return(
  d3
    .scaleLinear()
  ...
  function _yAxis(margin,d3,width){return(
  (g, y) =>
    g
  ...
  function _p90keyframes(getPData,keyframes,pDataSamplingRate){return(
  getPData(keyframes, 0.90, pDataSamplingRate)
  )}
  
  function _p50keyframes(getPData,keyframes,pDataSamplingRate){return(
  getPData(keyframes, 0.5, pDataSamplingRate)
  )}
  ...
  function _Tooltip(height,margin,y){return(
  class Tooltip {
    constructor(parent) {
      this.g = parent
        .append("g")
