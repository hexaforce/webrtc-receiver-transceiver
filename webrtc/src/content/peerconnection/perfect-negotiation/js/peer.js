...
function peer(other, polite, fail = undefined) {
  ...
  const assert_equals = !window.assert_equals ? (a, b, msg) => a === b || void fail(new Error(`${msg} expected ${b} but got ${a}`)) : window.assert_equals
  ...
  const commands = {
    async swapTransceivers() {
      ...
      if (transceiversForSending.length == 0) {
        ...
        transceiversForSending.push(pc.addTransceiver(stream1.getTracks()[0], { streams: [stream1], direction: 'sendonly' }))
        transceiversForSending.push(pc.addTransceiver('video', { streams: [stream2], direction: 'inactive' }))
        return
      }
    ...
    window.onmessage = async ({ data: { description, candidate, run } }) => {
      try {
        if (description) {
          ...
          // ignore the offer.
          const isStable = pc.signalingState == 'stable' || (pc.signalingState == 'have-local-offer' && srdAnswerPending)
          ignoreOffer = description.type == 'offer' && !polite && (makingOffer || !isStable)
          if (ignoreOffer) {
