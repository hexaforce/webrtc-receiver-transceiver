...
function getFinalResult() {
  ...
  if (servers.length === 1) {
    ...
    if (server.urls[0].indexOf('turn:') === 0 && server.urls[0].indexOf('?transport=tcp') === -1) {
  ...
  if (event.candidate) {
    ...
    if (['srflx', 'relay'].includes(candidate.type) && !candidate.url) {
      ...
      stats.forEach((report) => {
        if (!url && report.type === 'local-candidate' && report.address === candidate.address && report.port === candidate.port) {
          url = report.url
        }
      })
    }
...
function iceCandidateError(e) {
  ...
  document.getElementById('error').innerText += 'The server ' + e.url + ' returned an error with code=' + e.errorCode + ':\n' + e.errorText + '\n'
}
