...
remoteVideo.addEventListener('resize', (ev) => {
  ...
  console.log(elapsed, ': Resize event, size ', remoteVideo.videoWidth, 'x', remoteVideo.videoHeight)
  if (localVideo.videoWidth == remoteVideo.videoWidth && localVideo.videoHeight == remoteVideo.videoHeight) {
...
function gotDescription1(desc) {
  ...
  pc1.setLocalDescription(desc).then(() => {
    pc2.setRemoteDescription(desc).then(() => pc2.createAnswer().then(gotDescription2, onCreateSessionDescriptionError), onSetSessionDescriptionError)
...
function onIceCandidate(event) {
  getOtherPc(this).addIceCandidate(event.candidate).then(onAddIceCandidateSuccess).catch(onAddIceCandidateError)
  ...
  return pc1
    ...
    .then(() => {
      const desc = {
        ...
        sdp: bandwidthInKbps === 'unlimited' ? removeBandwidthRestriction(pc1.remoteDescription.sdp) : updateBandwidthRestriction(pc1.remoteDescription.sdp, bandwidthInKbps),
      }
