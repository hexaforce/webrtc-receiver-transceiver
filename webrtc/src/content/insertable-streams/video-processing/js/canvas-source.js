...
const TEXT_SOURCE = 'https://raw.githubusercontent.com/w3c/mediacapture-insertable-streams/main/explainer.md'
...
class CanvasSource {
  ...
  constructor() {
    ...
    fetch(TEXT_SOURCE)
      ...
      .catch((e) => {
        console.log(`[CanvasSource] The request to retrieve ${TEXT_SOURCE} encountered an error: ${e}.`)
      })
  }
  /** @override */
  setDebugPath(path) {
  ...
  updateCanvasVisibility() {
    if (this.canvas_.parentNode && !this.visibility_) {
      ...
      outputVideoContainer.parentNode.insertBefore(this.canvas_, outputVideoContainer)
    }
  }
  /** @private */
  requestAnimationFrame() {
    this.requestAnimationFrameHandle_ = requestAnimationFrame((now) => this.animate(now))
  }
  /**
   * @private
  ...
  animate(now) {
    ...
    const firstLineIdx = Math.floor(linesIncludingExtraBlank * (currentFrame / totalAnimationLength) - linesShown)
  ...
  async getMediaStream() {
    ...
    this.ctx_ = /** @type {?CanvasRenderingContext2D} */ (this.canvas_.getContext('2d', { alpha: false }))
    if (!this.ctx_) {
      throw new Error('Unable to create CanvasRenderingContext2D')
    }
    this.updateCanvasVisibility()
    this.stream_ = this.canvas_.captureStream(0)
    this.captureTrack_ = /** @type {!CanvasCaptureMediaStreamTrack} */ (this.stream_.getTracks()[0])
    this.requestAnimationFrame()
    console.log('[CanvasSource] Initialized canvas, context, and capture stream.', `${this.debugPath_}.canvas_ =`, this.canvas_, `${this.debugPath_}.ctx_ =`, this.ctx_, `${this.debugPath_}.stream_ =`, this.stream_, `${this.debugPath_}.captureTrack_ =`, this.captureTrack_)
