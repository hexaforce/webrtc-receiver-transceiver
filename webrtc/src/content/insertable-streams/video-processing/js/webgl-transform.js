...
class WebGLTransform {
  ...
  async init() {
    ...
    const gl = /** @type {?WebGLRenderingContext} */ (this.canvas_.getContext('webgl'))
    if (!gl) {
      alert('Failed to create WebGL context. Check that WebGL is supported ' + 'by your browser and hardware.')
      return
    }
    this.gl_ = gl
    const vertexShader = this.loadShader_(
      gl.VERTEX_SHADER,
      `
    ...
    const fragmentShader = this.loadShader_(
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
      varying vec2 texCoord;
    ...
    console.log('[WebGLTransform] WebGL initialized.', `${this.debugPath_}.canvas_ =`, this.canvas_, `${this.debugPath_}.gl_ =`, this.gl_)
  }
