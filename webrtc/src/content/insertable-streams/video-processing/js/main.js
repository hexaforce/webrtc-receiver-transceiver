...
if (typeof MediaStreamTrackProcessor === 'undefined' || typeof MediaStreamTrackGenerator === 'undefined') {
  alert('Your browser does not support the experimental MediaStreamTrack API ' + 'for Insertable Streams of Media. See the note at the bottom of the ' + 'page.')
}
...
function createProcessedMediaStreamTrack(sourceTrack, transform, signal) {
  ...
  promise.catch((e) => {
    if (signal.aborted) {
      console.log('[createProcessedMediaStreamTrack] Shutting down streams after abort.')
    } else {
      console.error('[createProcessedMediaStreamTrack] Error from stream transform:', e)
    }
    source.cancel(e)
    sink.abort(e)
  })
  ...
  console.log('[createProcessedMediaStreamTrack] Created MediaStreamTrackProcessor, ' + 'MediaStreamTrackGenerator, and TransformStream.', 'debug.processor =', processor, 'debug.generator =', generator, 'debug.transformStream =', transformer)
 ...
 */
function initUI() {
  const sourceSelector = /** @type {!HTMLSelectElement} */ (document.getElementById('sourceSelector'))
  const sourceVisibleCheckbox = /** @type {!HTMLInputElement} */ (document.getElementById('sourceVisible'))
  /**
  ...
  function updatePipelineSourceVisibility() {
    console.log(`[UI] Changed source visibility: ${sourceVisibleCheckbox.checked ? 'added' : 'removed'}`)
    if (pipeline) {
  ...
  const transformSelector = /** @type {!HTMLSelectElement} */ (document.getElementById('transformSelector'))
  /**
  ...
  function updatePipelineTransform() {
    ...
    const transformType = transformSelector.options[transformSelector.selectedIndex].value
  ...
  const sinkSelector = /** @type {!HTMLSelectElement} */ (document.getElementById('sinkSelector'))
  /**
  ...
  function initPipeline() {
    ...
    updatePipelineSink()
    console.log('[initPipeline] Created new Pipeline.', 'debug.pipeline =', pipeline)
  }
}
