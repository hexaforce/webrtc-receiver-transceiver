...
class PeerConnectionPipe {
  ...
  async init_(inputStream) {
    console.log('[PeerConnectionPipe] Initiating peer connection.', `${this.debugPath_} =`, this)
    ...
    await this.callee_.setRemoteDescription(/** @type {!RTCSessionDescription} */ (this.caller_.localDescription))
    await this.callee_.setLocalDescription()
    await this.caller_.setRemoteDescription(/** @type {!RTCSessionDescription} */ (this.callee_.localDescription))
    await receiverStreamPromise
    console.log('[PeerConnectionPipe] Peer connection established.', `${this.debugPath_}.caller_ =`, this.caller_, `${this.debugPath_}.callee_ =`, this.callee_)
