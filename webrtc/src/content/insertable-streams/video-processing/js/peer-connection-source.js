...
class PeerConnectionSource {
  ...
  async getMediaStream() {
    ...
    console.log('[PeerConnectionSource] Obtaining original source media stream.', `${this.debugPath_}.originalStreamSource_ =`, this.originalStreamSource_)
    const originalStream = await this.originalStreamSource_.getMediaStream()
    this.pipe_ = new PeerConnectionPipe(originalStream, `${this.debugPath_}.pipe_`)
    const outputStream = await this.pipe_.getOutputStream()
    console.log('[PeerConnectionSource] Received callee peer connection stream.', outputStream)
